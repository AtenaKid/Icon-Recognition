...
void iconRecog::testWithRealData() {
	...
	for (int i = 0; i < testFileName.size(); i++) {
		...
		Mat img, img_gray;
		img = imread(testFileName[i]);		//imshow("Org Image", img); waitKey(0);

		// Normalized the image, TODO --> simplify to same scale (white backgound, black color, remove the boundary)
		cvtColor(img, img_gray, CV_RGB2GRAY);
		img_gray = crop(img_gray);			//imshow("Cropped Image", img_gray); waitKey(0);
		img_gray = squalize(img_gray);		//imshow("Squalize Image", img_gray); waitKey(0);

		resize(img_gray, img_gray, Size(WIN, WIN), 0, 0, CV_INTER_LANCZOS4); imshow("Resized img", img_gray); waitKey(0);
		...
		*/
		...
		Point2d p2d_geometricCentre = CenroidEst(img_gray);			// 2 features 
		Point2d p2d_massCnetre = CentralMass(img_gray);

		vector<float> vi_symmetry = SymetryEst(img_gray);
		// ===========================================
...
// ========================================================== 
/* Calculate shape-based features Centroid, 
* Input is a matrix and output is two features vector 
*/
Point2d iconRecog::CenroidEst(Mat m_Img)
{	
	Moments m = moments((m_Img >= 50), true);
	Point2d p2d_geometricCentre(m.m10 / m.m00, m.m01 / m.m00);
	return p2d_geometricCentre;
}

Point2d iconRecog::CentralMass(Mat m_Img)
{
	Moments m = moments(m_Img, true);
	Point2d p2d_massCentre(m.m10 / m.m00, m.m01 / m.m00);
	return p2d_massCentre;
}

/* checking the symmetry condition 
*/
vector<float> iconRecog::SymetryEst(Mat m_Img)
{
	int row = m_Img.rows; 
	int col = m_Img.cols; 
	Mat  m_bImg = m_Img > 150; 

	// checking horizontal symmetry
	Mat tmp;
	flip(m_bImg, tmp, 0);
	float m_fErrHor = sqrt(norm(m_bImg - tmp));
	flip(m_bImg, tmp, 1);
	float m_fErrVer = sqrt(norm(m_bImg - tmp));

	// for diagonal symmetry 
	Point2f pc(col/2.0, row/ 2.0);
	Mat m_mRotateImg;
	Mat r = getRotationMatrix2D(pc, -90, 1.0);
	warpAffine(m_Img, m_mRotateImg, r, m_Img.size());

	Mat m_bRotateImg = m_mRotateImg > 150; 

	flip(m_bRotateImg, tmp, 0);
	float m_fErrDiag1 = sqrt(norm(m_bImg - tmp));
	flip(m_bRotateImg, tmp, 1);
	float m_fErrDiag2 = sqrt(norm(m_bImg - tmp));
	
	vector<float> vi_sym[] = { m_fErrHor, m_fErrVer, m_fErrDiag1, m_fErrDiag2 };
	return vi_sym;
}